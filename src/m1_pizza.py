...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    expected = [rg.Point(35.0, 10.0),
                rg.Point(25.0, 20.0),
                rg.Point(15.0, 10.0),
                rg.Point(25.0, 0.0)]
    circle = rg.Circle(rg.Point(25, 10), 10)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'Test 4: 60 purple circles'
    window = rg.RoseWindow(800, 600, title)
    circle = rg.Circle(rg.Point(400, 300), 100)
    circle.fill_color = 'midnight blue'
    draw_points_on_circle(window, circle, 60, 'lavender')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    circle_center = generate_points_on_circle(circle, number_of_points)
    circle.attach_to(window)
    for k in range(len(circle_center)):
        circle_small = rg.Circle(circle_center[k], 10)
        circle_small.fill_color = color
        circle_small.attach_to(window)
        circle_center[k].attach_to(window)
        window.render(0.01)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'Test 4: star in the sky'
    window = rg.RoseWindow(800, 600, title)

    circle = rg.Circle(rg.Point(400, 300), 200)
    circle.fill_color = 'midnight blue'
    pizza(window, circle, 100, 'white', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    start = generate_points_on_circle(circle, number_of_slices)
    circle.attach_to(window)
    for k in range(len(start)):
        line = rg.Line(start[k], circle.center)
        line.color = color
...
def run_test_polygon():
    ...
    # Test 3
    title = 'Test 3: its got many lines, coral pink'
    window = rg.RoseWindow(800, 600, title)

    circle = rg.Circle(rg.Point(200, 350), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 72, 'pink2', 6)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    #    and then draw lines that are based in part on those points.
    # ------------------------------------------------------------------
    shape_points = generate_points_on_circle(circle, number_of_segments)
    circle.attach_to(window)

    for k in range(number_of_segments):
        line = rg.Line(shape_points[k], shape_points[(k + 1) % number_of_segments])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render(0.01)
    # final_line = rg.Line(shape_points[len(shape_points) - 1], shape_points[0])
    # final_line.color = color
    # final_line.thickness = thickness
...
def run_test_fancy_polygon():
    ...
    # Test 4
    title = 'FANCY 4: whatever i chose'
    window = rg.RoseWindow(800, 600, title)
    circle = rg.Circle(rg.Point(400, 300), 200)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    ####################################################################
    # ------------------------------------------------------------------
    shape_points = generate_points_on_circle(circle, number_of_lines)
    circle.attach_to(window)

    for k in range(len(shape_points)):
        line = rg.Line(shape_points[k], shape_points[(k + hops_to_next_point) % number_of_lines])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render(0.01)
